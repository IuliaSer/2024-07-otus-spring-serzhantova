...
public class TestDataUtils {
    ...
    public static List<Author> getDbAuthors() {
        return IntStream.range(1, 4).boxed()
                .map(id -> new Author(id.toString(), "Author_" + id.toString()))
                .toList();
    }

    public static List<Genre> getDbGenres() {
        return IntStream.range(1, 4).boxed()
                .map(id -> new Genre(id.toString(), "Genre_" + id.toString()))
                .toList();
    }

    public static List<Book> getDbBooks(List<Author> dbAuthors, List<Genre> dbGenres) {
        return IntStream.range(1, 4).boxed()
                .map(id -> new Book(id.toString(), "BookTitle_" + id.toString(), dbAuthors.get(id - 1), dbGenres.get(id - 1)))
                .toList();
    }
    ...
    public static List<Comment> getDbComments(List<Book> dbBooks) {
        return IntStream.range(1, 4).boxed()
                .map(id -> new Comment(id.toString(), "Message_" + id.toString(), dbBooks.get(id - 1)))
                .toList();
    }
